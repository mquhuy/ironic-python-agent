...
class FakeIPA(base.ExecuteCommandMixin):
    ...
    def get_status(self):
        ...
        :returns: a :class:`ironic_python_agent.agent.IronicPythonAgent`fake
    ...
    def start_heartbeat(self):
        LOG.info("e UUID: %s", self.node_uuid)
    ...
    def execute_command(self, command_name, **kwargs):
        LOG.info('Executing command: %(name)s with args: %(args)s',
                  {'name': command_name,
                   'args': kwargs})
        if command_name == "start_node" and not self.power_on:
            self.power_on = True
            self.node_uuid = kwargs.get("uuid")
            self.start_heartbeat()

    def get_status(self):
        """
        For now, only return agent_token and uuid if exist
        """
        if self.power_on:
            return {"node_uuid": self.node_uuid, "agent_token": self.agent_token}
        return {}
